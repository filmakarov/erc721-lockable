...
For example, if you want to borrow some $DAI, you have to provide some $ETH as a collateral for a loan. During the loan period $ETH is being locked into the lending service contract. And it's ok for $ETH and other fungible tokens. 

It's different for NFTs. NFTs have plenty of use cases, that require for the NFT to stay on holder's wallet even when it is used as a collateral for a loan.
You may want to keep using your NFT as verified PFP on Twitter. You may want to use it to authorize on Discord server through Collab.land. You may want to use your NFT in a P2E game. And you should be able to do all of this even during lending period, like you are able to live in your house even it is mortgaged.


## Motivation
Initial idea was to just make NFTs that will feature better UX used as a collateral.
But then it became obvious, that one single locking feature allows for plenty of other usecases, such as lending/borrwowing NFT without a need for a collateral, paying for NFT by instalments, safe and convenient usage with hot wallets, non-custodial staking and much more.
Every usecase can (and some of them [are](https://github.com/mattdf/ERC721Loanable) already) be implemented one at a time.
Our aim however was to come up with a standardized implementation.

This approach proposes a solution which is designed to be as minimal as possible. At the same time it is a generalized impelementation, that allows for a lot of extensibility and potential use cases.
...
```
`getLocked` mapping serves to store the unlocker's addresses for the locked tokens. When there is zero adddress as value for the given key (tokenId), that means this token is not locked.

Both `_lock` and `_unlock` internal functions, that are implemented in the standard `ERC721s.sol` itself, do not perform any check on who can actually lock and unlock. They just do the job required.
All the checks are meant to be done in the implementation contract. It's because different projects can require for different approach for that. 
In this repo, `MockNFT.sol` implementation allows locking by holder and approved parties. However, some projects can limit it to only holder.
At the same time, implementation provided allows for unlocking only by unlocker stated in the `getLocked` mapping. However, other projects can choose to allow unlocking for some extra contract.

Aside of one mapping and two functions, there is a change to the `transferFrom` function. 
```
...
## Rationale (Usecases)
- **NFT-collateralised loans** Use your NFT as a collateral for a loan without locking it on the lending protocol contract. Lock it on your wallet instead and continue enjoying all the utility of your NFT.
- **No collateral rentals of NFTs** Borrow NFT for a fee, without a need for a huge collateral. You can use NFT, but not transfer it, so the lender is safe. Borrowing service contract automatically transfers NFT back to the lender as soon as borrowing period expires.
...
- **Secondary sales** Buy and sell your NFT by instalments. Buyer gets locked NFT and immediately starts using it. At the same time he/she is not able to sell the NFT until all the instalments are paid. If a full payment is not received, NFT goes back to the seller together with a fee.
- **S is for Safety** Use your exclusive blue chip NFTs safely and conveniently. The most convenient way to use NFT is together with MetaMask. However, MetaMask is vulnerable to various bugs and attacks. With `ERC721s` you can lock your NFT and declare your safe cold wallet as unlocker. Thus, you can still keep your NFT on MetaMask and use it conveniently. Even if hacker gets access to your MetaMask, they won’t be able to transfer your NFT without access to the cold wallet. That’s what makes `ERC721s` safe. This usecase is also [described](https://github.com/OwlOfMoistness/erc721-lock-registry) by OwlOfMoistness.
...
I'm sure that there will be more of usecases introduced as soon as community starts to explore `ERC721s`.

## Backwards Compatibility
A great number of legacy collections has been launched before `ERC721s`. So, there is obviously a need for a backwards compatibility solution to allow holders of existing blue chip collections enjoy services based on `ERC721s` locking feature.
Our approach is that such a solution should be based on the concept of Wrapping.
Blue chip collection admins setup a wrapping contract (that aside of locking can implement permits for gasless listings or on-chain royalties) and manage it, so the source of trust for holders does not change.
...
`permitLock` function implements `EIP26212`-like signature verification, that allows for better UX when using with actual service contracts. 

`MockLockerContract.sol` contains a sample of usage of the `permitLock` function by external contracts.

This implementation allows for a locker who has a permit to decide, what to state as an unlocker when calling `lock` function. 
Other implementations can limit this, by allowing locker to only state itself as an unlocker.

## Security Considerations
As soon as standard only introduces locking, there are few things to be considered security-wise.
...
Another issue, that is common to all the implementation of locked NFTs concept is that despite locked NFTs can not be transferred, nothing prevents a holder from listing it on OpenSea or other marketplace.
...
2. Later on, when there will be many projects implementing `ERC721s`, it will be easy for marketplaces to check if the token is locked or not by just calling public `getLocked` function. If the token is locked, the "Purchase" button can be deactivated and/or Lock icon can be shown next to such an asset. 
...
[ERC721LockRegistry](https://github.com/OwlOfMoistness/erc721-lock-registry) allows contract that implement the `ILock` interface to lock/unlock assets in place to enable/disable them from being transferred. This implementation allows multiple locks being put on the same asset, so the very same NFT can be used in multiple services simultaneously still living on a holder's wallet.
